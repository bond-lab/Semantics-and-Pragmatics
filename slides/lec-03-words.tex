...
\MyLogo{}
\begin{itemize}
\item An \txx{agentive noun} is a word that is derived from another word
  ...
  \begin{exe}
    ...
    \ex ?? \lex{undertaker, cooker, footballer}
  \end{exe}
...
\item Japanese (suffix distinguishes person/machine)
  \begin{itemize}
  \item \zh{運転する} → \zh{運転者}
  \item \zh{計算する} → \zh{計算者} \zh{計算機}
  \item \zh{研究する} → \zh{研究者} \zh{研究員}
  \item \zh{読む} → \zh{読み手} \zh{読者}
  \end{itemize}
...
\item Endings can mark gender, similar to pronouns
  \begin{itemize}
  \item Male Singer \tam{பாடகன்}   Pāṭagan
\\    Female singer  \tam{பாடகி} (Pāṭaki)
\\ generally female form of words simply add an இ (i).
\\    Formal Singer  \tam{பாடகர்}
  \item  he \tam{அவன்} (Avaṉ)
  \\  She  \tam{அவள்} (Avaḷ)
  \\  Formal/Gender-neutral  \tam{அவர்} (Avar)
  \end{itemize}
\end{itemize}
