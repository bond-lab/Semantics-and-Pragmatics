...
\begin{itemize}
\item \txx{voice} (passive, middle) changes the number of arguments.
\begin{exe}
...
\myslide{Alternations have semantic consequences}

\begin{itemize}
\item Son far we sought to describe alternations in terms of \txx{changes to their theta grids}
\item Alternations lead to fine-grained differences in the meanings of predicates \txx{not necessarily captured in theta grids}
\item Consider the difference between passives and the causative-inchoative alternation

\end{itemize}

\myslide{Optional agents}
...
\ex \begin{xlist}
  \ex \eng{Kim ate Sandy.}\\
  \grid{\ul{agent}, patient, instrument}
\ex \eng{Sandy was eaten (by Kim).}\\
  \grid{\ul{patient}}  $\rightarrow$ agent becomes optional adjunct not in theta-grid
\end{xlist}

\ex \begin{xlist}
  \ex \eng{Kim broke the window with a hammer.}\\
  \grid{\ul{agent}, patient, instrument}
\ex \eng{The window broke *by Kim / *by the hammer.}\\
  \grid{\ul{patient}}  $\rightarrow$ agent or instrument cannot be realized
\end{xlist}

\end{exe}

\myslide{Internally caused change}
...
\begin{exe}
  \ex \begin{xlist}
    \ex \eng{The window was broken *by itself.}
    \ex \eng{The window broke by itself.}
    \\Both \grid{\ul{patient}}?
  \end{xlist}
\end{exe}

\myslide{Back to the middle}
...
\begin{exe}
  \ex \begin{xlist}
    \ex \eng{The window was broken (by Sandy).}
    \ex \eng{The window broke (by itself).}
    \ex \eng{The window breaks easily.}
    \\All \grid{\ul{patient}}!
  \end{xlist}
\end{exe}

\myslide{How to realize the middle's agent}
...
\begin{exe}
  \ex \begin{xlist}
    \ex \eng{The window was broken by Sandy.}
    \\\grid{\ul{patient}}
    \ex \eng{The window broke by itself / *by Sandy.} \\\grid{\ul{patient}}
    \ex \eng{The window breaks easily *by children / \ul{for} children.} \\\grid{\ul{patient}}
    \ex \eng{This knife cuts easily \ul{for} Kim.} \\\grid{\ul{instrument}}
  \end{xlist}
\end{exe}

\myslide{Participant changing alternations}
\begin{itemize}
\item So far the alternations we see simply remove one participant role from theta grids
\item Other alternations can in fact \txx{change the array of participants within a theta grid}
\item This is most prominent in English with ditransitive verbs
\end{itemize}

\myslide{Double object and dative alternations}
\begin{itemize}
...
\item Intuitively, both variants mean something like \txx{cause goal to possess theme}
\end{itemize}

\begin{exe}
  \ex \begin{xlist}
    \ex \eng{Kim sent Sandy a letter.}
    \\\grid{\ul{agent}, goal, theme}
    \ex \eng{Kim sent a letter to Sandy.} \\\grid{\ul{agent}, goal, beneficiary}
  \end{xlist}
\end{exe}

\myslide{Can change participant roles}
\begin{itemize}
\item But the dative frame is compatible also with something like \txx{cause theme to move to location} where location is strictly a non-animate place
...
\begin{exe}
  \ex \begin{xlist}
    \ex \eng{*Kim sent London a letter.}
    \\\grid{\ul{agent}, location, theme}
    \ex \eng{Kim sent a letter to London.} \\\grid{\ul{agent}, theme, location}
  \end{xlist}
\end{exe}
\end{itemize}

\myslide{Verb sensitivity}
\begin{itemize}
...
\begin{exe}
  \ex \begin{xlist}
    \ex \eng{Kim gave Sandy a book.}
    \\\grid{\ul{agent}, goal, theme}
    \ex \eng{Kim gave a book to Sandy.} \\\grid{\ul{agent}, theme, goal}
  \end{xlist}
\end{exe}

\begin{exe}
  \ex \begin{xlist}
    \ex \eng{*Kim gave London a book.}
    \\\grid{\ul{agent}, location, theme}
    \ex \eng{*Kim gave a book to London.} \\\grid{\ul{agent}, theme, location} $\rightarrow$ compare dative variant of \emph{send}
  \end{xlist}
\end{exe}


\myslide{More on ditransitive alternations}
\begin{itemize}
...
\begin{exe}
  \ex \begin{xlist}
    \ex \eng{Kim taught Sandy French \# but Sandy never learnt it.}
    \hspace*{\fill}\grid{\ul{agent}, goal, theme}
    \ex \eng{Kim taught French to Sandy but Sandy never learnt it.} \hspace*{\fill}\grid{\ul{agent}, theme, goal} 
  \end{xlist}
\end{exe}


\myslide{Verb sensitivity again}
\begin{itemize}
...
\begin{exe}
  \ex \begin{xlist}
    \ex \eng{Kim gave Sandy a book \# but Sandy never got it.}
    \ex \eng{Kim gave a book to Sandy \# but Sandy never got it.}
  \end{xlist}
\end{exe}

\begin{exe}
  \ex \begin{xlist}
    \ex \eng{Kim sent Sandy a book but Sandy never got it.}
    \ex \eng{Kim sent a book to Sandy but Sandy never got it.}
  \end{xlist}
\end{exe}

\myslide{Alternations, participant roles, and theta grids}
\begin{itemize}
